...
import { ExamBlueprintPanel } from './panels/ExamBlueprintPanel';
...
export const PanelViews: React.FC<PanelViewsProps> = ({ activePanel, currentUser, token }) => {
  ...
  if (panel === 'reports') return <ReportsPanel currentUser={currentUser} schools={schools} students={students} reportsList={reportsList} token={token} />;

  if (panel === 'exam_blueprint') return <ExamBlueprintPanel token={token} />;
