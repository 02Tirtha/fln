...
export class RemediationService {
  ...
  async startGeneration(studentId: string, examId: string, failedQuestionNums: number[], originalQuestions?: any[]): Promise<{ ledgerId: string; status: string }> {
    ...
    const responses = await Promise.all(
      failedQuestionNums.map(async (qNo, listIdx) => {
        let originalInfo: any = {};
        if (originalQuestions && Array.isArray(originalQuestions)) {
          // The frontend sends only the failed questions, each with a questionNumber field.
          // Try to match by questionNumber first, then fall back to positional lookup.
          const q = originalQuestions.find((x: any) =>
            x.questionNumber === qNo ||
            x.questionNo === qNo ||
            x.question_no === qNo
          ) || originalQuestions[listIdx];

          if (q) {
            const questionText = q.question || q.questionText || q.originalQuestion || '';
            const answerValue = typeof q.answer === 'object' ? JSON.stringify(q.answer) : String(q.answer ?? q.correctAnswer ?? '');
        ...
        // Only fall back to DB lookup if we got no real question text at all
        ...
        return {
          ...
          isCorrect: false,
          practiceQuestions: (await this.getInlineFallback(
        ...
        try {
          ...
          if (studentId) {
            try {
              ...
              console.warn('[Remediation] Failed to check student classGroup in background generation:', err);
            }
          }
          ...
          if (!batch || batch.length === 0) {
            ...
            batch = await this.getInlineFallback(origQ, concept, qType, baseOffset, studentId, response.originalAnswer || response.answer || '');
          }
          ...
          response.practiceQuestions = (await this.getInlineFallback(
  ...
  private async findOriginalQuestion(examId: string, questionNumber: number): Promise<{
    ...
    try {
      ...
      if (resolvedExamId.startsWith('rem_')) {
        const ledgers = await dbStore.getRemediationLedgers();
        const ledger = ledgers.find(l => l.id === resolvedExamId);
