...
const generateOrdinalNumbers: ConceptGenerator = (v, originalQ = '', originalAnswer = '', classLevel = 5) => {
...
const generateMissingNumbers: ConceptGenerator = (v, originalQ = '') => {
  ...
  const sets = Array.from({ length: 5 }, (_, i) => {
    ...
    const answer = seq[2];
    seq[2] = '___' as any;
    return { prompt: `${seq.join(', ')}`, answer: String(answer) };
  });
  return { question: "Fill in the missing number:", subQuestions: sets, answer: "", topic: "Missing Numbers", aiGenerated: false };
};
