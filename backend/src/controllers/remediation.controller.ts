...
export class RemediationController {
  ...
  async generate(req: Request, res: Response): Promise<void> {
    try {
      ...
      const questionIds: string[] = Array.isArray(failedQuestionIds) && failedQuestionIds.length > 0
      ...
      if (!studentId || !examId || questionIds.length === 0) {
        res.status(400).json({ success: false, error: 'Missing studentId, examId, or failed question IDs.' });
        return;
      }
  ...
  async getLedgerByStudentAndExam(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log('[Remediation GET] LOOKUP', {
        studentId,
        examId
      });
      console.log('[Remediation GET] Mongoose Query:', { studentId, examId });
      
      try {
        if (mongoose.connection.readyState === 1) {
          ledger = await RemediationLedger.findOne({ studentId, examId }).exec();
          console.log('[Remediation GET] Mongoose collection queried:', RemediationLedger.collection.collectionName);
        }
      } catch (err) {
      ...
      if (!ledger) {
        console.log('[Remediation GET] Querying dbStore...');
        ledger = await dbStore.getRemediationLedgerByStudentAndExam(studentId, examId);
      }

      if (!ledger) {
        console.error('[Remediation GET] Remediation record NOT FOUND');
        res.status(404).json({ success: false, error: 'Remediation ledger not found for this student and exam.' });
        return;
      }

      // console.log('[Remediation GET] RESULT FOUND', ledger.id, ledger.remediationStatus);
      ...
      if (ledger.remediationStatus !== 'generating' && ledger.remediationStatus !== 'pending') {
        for (const r of ledger.responses || []) {
          ...
          if (isDuplicateFlat || isMissingSub || isUnderlinedMismatch || pqs.length === 0) {
            ...
            r.practiceQuestions = await remediationService.getInlineFallback(r.originalQuestion, r.conceptName, r.questionType || 'standard', 0, ledger.studentId, r.originalAnswer || '') as any;
            needSave = true;
          }
        }
      }
  ...
  async getBatchLedgers(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        ledgers = all.filter(l => l.remediationStatus === 'completed');
      }
  ...
  async getLedgersForStudent(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn('Mongoose query failed, searching dbStore:', err);
        ledgers = await dbStore.getRemediationLedgersByStudent(studentId);
      }
