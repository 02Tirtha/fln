...
const reviewAssessmentPapers = [
  ...
  },
  {
    ...
    questions: [
      ...
      'Data & Time (count-and-tally) — Item 1',
      'Data & Time (clock-read-grid) — Item 3',
      'Data & Time (ordinal-circle-write) — Item 3',
...
reviewAssessmentPapers.forEach((paperInput) => {
  ...
  paperOutput.topics.forEach((topic, idx) => {
    ...
    console.log(`5 PRACTICE REMEDIATION VARIANTS:`);

    topic.practiceQuestions.forEach((pq, qIdx) => {
      console.log(`  Variant ${qIdx + 1}: ${pq.question}`);
      if (pq.options && pq.options.length > 0) {
        console.log(`     Options: [${pq.options.join(', ')}]`);
      }
      console.log(`     Answer: ${pq.answer}`);
    });
  });
});
